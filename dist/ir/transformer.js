...
class IRTransformer {
    transform(circuit) {
        ...
        const bounds = {
            x: 0,
            y: 0,
            width: Math.max(800, circuit.components.length * 100),
            height: Math.max(600, circuit.components.length * 50) // Dynamic height based on components
        };
        // Transform components
        const components = circuit.components.map((component, index) => {
        ...
        // Transform wires
        const wires = this.transformWires(circuit, components);
        return {
            name: circuit.name,
            components,
            wires,
    ...
    generatePins(type, position) {
        ...
        switch (type) {
            ...
            case 'pnp':
                ...
                return [
                    { name: 'B', position: [position[0] + 30, position[1] - 30] },
                    { name: 'C', position: [position[0], position[1] + 30] },
    ...
    transformWires(circuit, components) {
