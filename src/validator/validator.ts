...
export class Validator {
  ...
  constructor() {
    // Add built-in rules
    this.addRule(new UniqueComponentIdRule());
    this.addRule(new ValidComponentTypeRule());
    this.addRule(new ValidNodeConnectionsRule());
    this.addRule(new NoFloatingNodesRule());
    this.addRule(new GroundNodeExistsRule());
    this.addRule(new ValidValueFormatRule());
    this.addRule(new ComponentExistsRule());
    this.addRule(new CircuitNameRule());
  }
  ...
  public validate(circuit: Circuit): ValidationError[] {
...
class UniqueComponentIdRule implements ValidationRule {
  validate(circuit: Circuit): ValidationError[] {
    const errors: ValidationError[] = [];
...
class ValidComponentTypeRule implements ValidationRule {
  validate(circuit: Circuit): ValidationError[] {
    const errors: ValidationError[] = [];
    const validTypes = [
...
class ValidNodeConnectionsRule implements ValidationRule {
  validate(circuit: Circuit): ValidationError[] {
    const errors: ValidationError[] = [];
...
class NoFloatingNodesRule implements ValidationRule {
  validate(circuit: Circuit): ValidationError[] {
    const errors: ValidationError[] = [];
...
class GroundNodeExistsRule implements ValidationRule {
  validate(circuit: Circuit): ValidationError[] {
    const errors: ValidationError[] = [];
...
class ValidValueFormatRule implements ValidationRule {
  validate(circuit: Circuit): ValidationError[] {
    const errors: ValidationError[] = [];
...
class ComponentExistsRule implements ValidationRule {
  validate(circuit: Circuit): ValidationError[] {
    const errors: ValidationError[] = [];
...
class CircuitNameRule implements ValidationRule {
  validate(circuit: Circuit): ValidationError[] {
    const errors: ValidationError[] = [];
