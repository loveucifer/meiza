...
use crate::components::{get_component_template, get_component_pins};
...
pub fn calculate_layout(circuit: &Circuit) -> Result<Layout, Box<dyn std::error::Error>> {
    ...
    for component in unpositioned {
        let position = determine_position(
            component,
            &positioned,
            &adjacency,
    ...
    for connection in &circuit.connections {
        let from_pos = positioned.get(&connection.from.component_id)
            .ok_or("Component not found")?;
        let to_pos = positioned.get(&connection.to.component_id)
            .ok_or("Component not found")?;
...
fn get_pin_position(
    ...
    let positioned_comp = positioned_components.iter()
        .find(|pc| pc.component.id == component_id)
        .ok_or("Component not found in positioned components")?;
    
    // Get the pin definition
    if let Some(template) = get_component_template(&format!("{:?}", positioned_comp.component.component_type).to_lowercase()) {
...
fn route_connection(
    ...
    if (to.x - current.x).abs() > 1.0 {
        ...
        path.push(Point { x: mid_x, y: current.y });
        current.x = mid_x;
    }
    
    if (to.y - current.y).abs() > 1.0 {
        path.push(Point { x: current.x, y: to.y });
        current.y = to.y;
    }
    
    if (to.x - current.x).abs() > 1.0 {
        path.push(to);
    } else if path.last() != Some(&to) {
        path.push(to);
    }
