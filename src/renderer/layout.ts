...
import { ComponentIR, WireIR, CircuitIR } from '../ir/types';

// Grid-based layout with auto-routing
export class LayoutEngine {
  private gridSize: number = 20; // pixels
  private margin: number = 50;   // pixels

  public layout(circuitIR: CircuitIR): CircuitIR {
    // Create a new circuit IR with updated positions
    const layoutIR: CircuitIR = {
      name: circuitIR.name,
      components: [],
      wires: [],
  ...
  private positionComponents(components: ComponentIR[], bounds: { width: number; height: number }): ComponentIR[] {
    ...
    return components.map((comp, index) => {
      ...
      const updatedPins = comp.pins.map((pin: { name: string; position: [number, number] }) => ({
        name: pin.name,
        position: [pin.position[0] + x - comp.position[0], pin.position[1] + y - comp.position[1]] as [number, number]
      }));
  ...
  private routeWires(components: ComponentIR[], originalWires: WireIR[]): WireIR[] {
  ...
  private calculateWirePath(from: [number, number], to: [number, number]): [number, number][] {
    ...
    return [
      from,
      [midX, from[1]], // Horizontal segment
      [midX, to[1]],   // Vertical segment
      to
    ];
  }

  // Additional utility methods for layout operations
  public snapToGrid(position: [number, number]): [number, number] {
